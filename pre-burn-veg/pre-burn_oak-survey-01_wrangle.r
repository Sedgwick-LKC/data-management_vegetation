## ------------------------------------------------ ##
# Pre-Burn Oak Survey - Wrangle
## ------------------------------------------------ ##
# Purpose:
## Wrangle the 'raw' pre-burn oak survey data
...
oak_file <- file.path("data", "raw", "Pre-burn-Oak-survey.xlsx")
...
oak_v02 <- oak_v01 %>% 
  ...
  dplyr::mutate(dplyr::across(.cols = dplyr::contains(c("tag", "location", "canopy", "dbh", "d50")),
    .fns = as.numeric)) %>% 
  # Fix date class
...
oak_v04 <- oak_v03 %>% 
  ...
  # Manual renaming
  dplyr::rename(oak.latitude_deg = location..latitude.,
    oak.longitude_deg = location..longitude.,
  canopy.alive_percent = canopy.alive)
...
(oak_tidyname <- paste0("pre-burn_oak-survey_",
